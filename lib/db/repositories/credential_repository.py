...
class CredentialRepository(BaseRepository):
    async def create(
        ...
        """Create a credential and auto-activate it if it is the provider's first one."""
    ...
    async def get_active_credentials_bulk(self) -> dict[str, ProviderCredential]:
        """Fetch active credentials for all providers in one query."""
        stmt = select(ProviderCredential).where(
            ProviderCredential.is_active == True,  # noqa: E712
        )
        result = await self.session.execute(stmt)
    ...
    async def activate(self, cred_id: int, provider: str) -> None:
        """Activate one credential and deactivate the provider's others."""
    ...
    async def update(
        ...
        *,
        ...
        """Update credential fields. ``base_url`` uses ``_UNSET`` to mean omitted."""
    ...
    async def delete(self, cred_id: int) -> None:
        """Delete a credential and promote the earliest remaining one if needed."""
        cred = await self.get_by_id(cred_id)
        if cred is None:
            return
