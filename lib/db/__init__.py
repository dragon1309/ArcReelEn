...
async def init_db() -> None:
    ...
    This uses ``Config()`` plus ``set_main_option`` programmatically instead of
    ``Config("alembic.ini")`` so ``env.py`` does not override the application's
    logging configuration via ``fileConfig()``.
    """
    import asyncio
    ...
    def _run_alembic():
        # Build ``Config`` programmatically to skip ``alembic.ini`` and protect
        # the app's logging configuration from ``env.py`` fileConfig().
