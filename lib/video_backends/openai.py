...
class OpenAIVideoBackend:
    ...
    async def generate(self, request: VideoGenerationRequest) -> VideoGenerationResult:
        ...
        logger.info("Starting OpenAI video generation: model=%s, seconds=%s", self._model, kwargs["seconds"])
        ...
        logger.info("Downloaded OpenAI video to %s", request.output_path)
    ...
    async def _create_video(self, **kwargs):
        """Create and poll video generation with its own retry policy."""
    ...
    async def _download_content_with_retry(self, video_id: str):
        """Retry content download separately so generation is not retriggered on download failure."""
