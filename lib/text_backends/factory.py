"""Factory helpers for creating text backends from runtime configuration."""
...
async def create_text_backend_for_task(
    ...
    """Create a text backend using DB-backed runtime configuration."""
    ...
    async with resolver.session() as r:
        ...
        if is_custom_provider(provider_id):
            ...
            async with r._open_session() as (session, _):
                ...
                if provider is None:
                    raise ValueError(
                        "The configured custom provider was deleted. Re-select a text model in project settings."
                    )
                name = provider.display_name
                # If the selected model no longer exists or is disabled, fall back to the default model.
                if model_id:
                ...
                if not model_id:
                    ...
                    if default_model:
                        ...
                        raise ValueError(
                            f'Provider "{name}" has no enabled text models. Re-select a model in project settings.'
                        )
