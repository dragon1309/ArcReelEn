...
class ArkTextBackend:
    ...
    def __init__(self, *, api_key: str | None = None, model: str | None = None):
        ...
        # Instructor requires an ``openai.OpenAI`` client. The Ark SDK client is
        # not compatible, so create a native OpenAI client for fallback usage.
    ...
    def _resolve_capabilities(self) -> set[TextCapability]:
        """Build the capability set from the model metadata in ``PROVIDER_REGISTRY``."""
        ...
        # If the model is unknown, prefer Instructor fallback over a native call
        # that may fail with unsupported structured output.
        return base
    ...
    async def _generate_structured(self, request: TextGenerationRequest) -> TextGenerationResult:
        ...
        if TextCapability.STRUCTURED_OUTPUT in self._capabilities:
            ...
            try:
                ...
                logger.warning("Native response_format failed (%s); falling back to Instructor/json_object", exc)
