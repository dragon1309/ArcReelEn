...
This module centralizes config reads and default definitions that used to live
across multiple files. Values are read from the database on each call instead
of being cached because the local SQLite overhead is negligible.
"""
...
def get_project_manager() -> ProjectManager:
    """Return the shared ProjectManager singleton for the standard project root."""
...
# Truthy string values accepted by boolean parsing.
...
class ConfigResolver:
    ...
    This is a thin layer over ``ConfigService`` that provides:
    - a single source of truth for defaults
    - typed outputs (bool / tuple / dict)
    - built-in precedence handling (global config -> project override)
    """
    ...
    async def session(self) -> AsyncIterator[ConfigResolver]:
        """Open a shared session and yield a resolver bound to that session."""
    ...
    async def _open_session(self) -> AsyncIterator[tuple[AsyncSession, ConfigService]]:
        """Yield ``(session, ConfigService)``, reusing the bound session first."""
        if self._bound_session is not None:
    ...
    async def video_generate_audio(self, project_name: str | None = None) -> bool:
        ...
        Precedence: project override > global config > default (False).
        """
    ...
    async def provider_config(self, provider_id: str) -> dict[str, str]:
        """Fetch the config for a single provider."""
        async with self._open_session() as (session, svc):
    ...
    async def all_provider_configs(self) -> dict[str, dict[str, str]]:
        """Fetch configs for all providers in one call."""
        async with self._open_session() as (session, svc):
            return await self._resolve_all_provider_configs(svc, session)

    # Internal resolution helpers (independently testable with a provided svc).
    ...
    async def text_backend_for_task(
        ...
        """Resolve the text backend using project, global, then auto defaults."""
        async with self._open_session() as (session, svc):
    ...
    async def _auto_resolve_backend(
        ...
        """Return the first ready provider that supports the requested media type."""
        ...
        raise ValueError(
            f"No available {media_type} provider found. Configure at least one provider in Settings > Providers."
        )
