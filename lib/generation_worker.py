...
def _project_level_provider(project: dict, task_type: str) -> str | None:
    ...
    Both video and image backends are parsed from ``video_backend`` /
    ``image_backend`` values in ``provider/model`` format.
    """
...
async def _extract_provider(task: dict[str, Any]) -> str:
    ...
    # Support already-enqueued legacy tasks that carry an explicit provider.
    provider = payload.get("video_provider") or payload.get("image_provider")
    if provider:
        return _normalize_provider_id(provider)
    # Resolve the effective provider from project config, then fall back to the global default.
...
async def _load_pools_from_db() -> dict[str, ProviderPool]:
    ...
    async with safe_session_factory() as session:
        ...
        # Load pool configs for custom providers using the same defaults as built-ins.
    ...
    logger.info(
        "Loaded provider pool configuration from the database: %s",
...
def _build_default_pools() -> dict[str, ProviderPool]:
    ...
    Create default pools for every provider in ``PROVIDER_REGISTRY`` so tasks
    claimed before DB-backed config is available do not fall back to a generic
    single-concurrency pool.
    """
    from lib.config.registry import PROVIDER_REGISTRY
...
class GenerationWorker:
    ...
    def _get_or_create_pool(self, provider_id: str) -> ProviderPool:
        ...
        if pool is not None:
            return pool
        # Unknown provider: use the same defaults as built-in providers.
        image_max = _read_int_env("IMAGE_MAX_WORKERS", 5, minimum=1)
        video_max = _read_int_env("VIDEO_MAX_WORKERS", 3, minimum=1)
        pool = ProviderPool(
            provider_id=provider_id,
        ...
        logger.info("Created fallback pool for provider %s (image=%d, video=%d)", provider_id, image_max, video_max)
        return pool
    ...
    async def reload_limits(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to reload provider configuration from the database; keeping current pools", exc_info=True)
            return
        ...
        logger.info(
            "Updated provider pool configuration: %s",
            {pid: (p.image_max, p.video_max) for pid, p in self._pools.items()},
        )
    ...
    async def _run_loop(self) -> None:
        try:
            while not self._stop_event.is_set():
                ...
                # Requeue orphaned running tasks only when this worker has just
                # acquired the lease and has no inflight work of its own. This
                # avoids requeuing its own tasks during brief lease flaps.
    ...
    async def _claim_tasks(self) -> bool:
        ...
        for media_type in ("image", "video"):
            ...
            while True:
                ...
                if max_capacity == 0:
                    # Provider does not support this media type. Fail fast instead
                    # of endlessly requeuing the same task.
                    logger.warning(
                ...
                if not has_room:
                    ...
                    logger.info(
                        "Provider %s %s pool is full; requeued task %s",
                        provider_id,
                        media_type,
                        task["task_id"],
                    )
    ...
    async def _requeue_single_task(self, task_id: str) -> None:
        ...
        try:
            ...
            logger.debug("Requeued task %s because the provider pool was full", task_id)
        except Exception:
            logger.warning("Failed to requeue task %s", task_id, exc_info=True)
        ...
        for pool in self._pools.values():
            for finished_task in pool.drain_finished():
                try:
                    ...
                    logger.debug("Task exception was already logged in _process_task")
    ...
    async def _process_task(self, task: dict[str, Any]) -> None:
        ...
        provider_id = await _extract_provider(task)
        logger.info("Started processing task %s (type=%s, provider=%s)", task_id, task_type, provider_id)
        try:
            ...
            logger.info("Completed task %s (type=%s, provider=%s)", task_id, task_type, provider_id)
