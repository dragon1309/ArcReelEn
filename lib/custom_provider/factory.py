"""Factory helpers for wrapped custom-provider backends."""
...
def create_custom_backend(
    *,
    ...
    """Create a wrapped backend instance from a ``CustomProvider`` record."""
    api_format = provider.api_format
    if api_format not in _VALID_API_FORMATS:
        raise ValueError(f"Unsupported api_format: {api_format!r}. Supported values: {_VALID_API_FORMATS}")
    if media_type not in _VALID_MEDIA_TYPES:
        raise ValueError(f"Unsupported media_type: {media_type!r}. Supported values: {_VALID_MEDIA_TYPES}")
...
def _create_openai_backend(
    *,
    ...
    """Create an OpenAI-compatible wrapped backend."""
...
def _create_google_backend(
    *,
    ...
    """Create a Google-compatible wrapped backend."""
