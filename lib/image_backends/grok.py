"""xAI Grok (Aurora) image-generation backend."""
...
def _validate_aspect_ratio(aspect_ratio: str) -> str:
    """Warn when an aspect ratio is not in the documented Grok allowlist."""
    if aspect_ratio not in _SUPPORTED_ASPECT_RATIOS:
        logger.warning("Grok may not support aspect_ratio=%s; passing it through to the API", aspect_ratio)
    return aspect_ratio


class GrokImageBackend:
    """xAI Grok (Aurora) image backend supporting both T2I and I2I."""
    ...
    async def generate(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        """Generate an image from text or reference images."""
        ...
        # In image-to-image mode, convert every reference image into a base64 data URI.
        if request.reference_images:
            ...
            if data_uris:
                ...
                logger.info("Grok I2I mode with %d reference images", len(data_uris))

        logger.info("Starting Grok image generation: model=%s", self._model)
        response = await self._client.image.sample(**generate_kwargs)

        # Moderation check.
        if not response.respect_moderation:
            raise RuntimeError("Grok image generation was rejected by moderation")

        # Download the image locally.
        await _download_image(response.url, request.output_path)

        logger.info("Downloaded Grok image to %s", request.output_path)
...
def _map_image_size_to_resolution(image_size: str) -> str:
    """Map generic image-size labels such as ``1K`` to Grok resolution values."""
    mapping = {
        "1K": "1k",
        "2K": "2k",
        "1k": "1k",
