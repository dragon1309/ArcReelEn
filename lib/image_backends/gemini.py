"""Gemini image-generation backend extracted from the older Gemini client."""
...
# Filename prefixes that should not be used for label inference.
...
class GeminiImageBackend:
    """Gemini image backend supporting both AI Studio and Vertex AI."""
    ...
    def __init__(
        ...
        *,
        ...
        if self._backend_type == "vertex":
            ...
            if not _api_key:
                raise ValueError("Gemini API key is missing. Configure it in Global Settings -> Providers.")
    ...
    async def generate(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        # 2. Build contents from reference images and the prompt.
    ...
    def _extract_name_from_path(image_path: str | Path) -> str | None:
        """Infer a name from the image path, skipping known generated-file prefixes."""
    ...
    def _build_contents_with_labeled_refs(
        ...
        """Build the contents list with optional text labels for each reference image."""
        contents: list = []
        ...
        # The prompt always comes last.
        contents.append(prompt)
        return contents

    @staticmethod
    def _process_image_response(response, output_path: Path) -> Image.Image:
        """Parse the image-generation response and save it to disk."""
