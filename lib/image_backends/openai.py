...
class OpenAIImageBackend:
    """OpenAI image backend supporting both T2I and I2I."""
    ...
    async def _generate_edit(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        if len(refs) > _MAX_REFERENCE_IMAGES:
            logger.warning("Reference image count %d exceeds the limit %d; truncating", len(refs), _MAX_REFERENCE_IMAGES)
        ...
        try:
            ...
            if not image_files:
                logger.warning("All reference images were invalid; falling back to T2I")
                return await self._generate_create(request)
    ...
    def _save_and_return(self, response, request: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        logger.info("OpenAI image generation finished: %s", request.output_path)
