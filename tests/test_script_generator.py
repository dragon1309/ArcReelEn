...
class _FakeTextGenerator:
    """Fake ``TextGenerator`` wrapper around ``_FakeTextBackend``."""
...
class TestScriptGenerator:
    async def test_build_prompt_uses_step1_content(self, tmp_path):
        """``build_prompt`` works without a client in dry-run mode."""
    ...
    async def test_generate_passes_pydantic_class_as_schema(self, tmp_path):
        """``generate`` should pass the Pydantic class instead of ``model_json_schema()``."""
        project_path = tmp_path / "demo"
        _write_json(
            project_path / "project.json",
            {
        ...
        generator = ScriptGenerator(project_path, generator=fake)
        # Validation will fail, but the schema should still be passed as a class.
    ...
    async def test_generate_without_backend_raises(self, tmp_path):
        """Calling ``generate()`` without a backend should raise ``RuntimeError``."""
        project_path = tmp_path / "demo"
        _write_json(project_path / "project.json", {"title": "Project"})
        _write(project_path / "drafts" / "episode_1" / "step1_segments.md", "content")

        generator = ScriptGenerator(project_path)  # No backend.
        with pytest.raises(RuntimeError, match="TextGenerator is not initialized"):
            await generator.generate(1)
