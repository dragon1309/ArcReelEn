"""
Video Project Management WebUI - FastAPI application.
...
async def lifespan(app: FastAPI):
    ...
    # Repair agent_runtime symlinks in existing projects.
    ...
    if any(v > 0 for v in _symlink_stats.values()):
        logger.info("agent_runtime symlink repair completed: %s", _symlink_stats)
...
app = FastAPI(
    ...
    description="Web management interface for the AI video generation workspace",
    version="1.0.0",
    lifespan=lifespan,
)
...
app.include_router(versions.router, prefix="/api/v1", tags=["Versioning"])
...
async def serve_skill_md(request: Request) -> Response:
    """Render the skill.md template with the current service base URL."""
    ...
    # Infer the base URL from the request and only trust x-forwarded-proto,
    # using the actual connection target for host instead of spoofable x-forwarded-host.
...
# Frontend build output: SPA static file service mounted after all explicit routes.
