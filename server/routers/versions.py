...
def get_version_manager(project_name: str) -> VersionManager:
    """Return the version manager for a project."""
...
def _resolve_resource_path(
    ...
    """Return ``(current_file_absolute, relative_file_path)`` for a resource."""
...
def _sync_metadata(
    ...
    """Sync metadata after restore so references point to the canonical file path."""
    if resource_type == "characters":
        try:
            with project_change_source("webui"):
                get_project_manager().update_project_character_sheet(project_name, resource_id, file_path)
        except KeyError:
            pass  # The character entry may already be gone from project.json.
    elif resource_type == "clues":
        try:
            with project_change_source("webui"):
                get_project_manager().update_clue_sheet(project_name, resource_id, file_path)
        except KeyError:
            pass  # The clue entry may already be gone from project.json.
...
async def get_versions(
    ...
    """Return the full version list for a resource."""
    try:
...
async def restore_version(
    ...
    """Restore a resource to the requested version."""
    try:
        ...
        def _sync():
            ...
            # Compute fingerprints after restore. For videos, also clear the stale thumbnail.
            ...
            if resource_type == "videos":
                ...
                # fingerprint=0 tells the frontend the thumbnail is now invalid.
