...
Export the generated video clips for a single ArcReel episode as a Jianying
draft ZIP. The service uses pyJianYingDraft to generate draft_content.json and
then rewrites paths so the draft points to the user's local Jianying directory.
"""
...
class JianyingDraftService:
    ...
    def _find_episode_script(self, project_name: str, project: dict, episode: int) -> tuple[dict, str]:
        """Find the script file for the requested episode and return ``(script_dict, filename)``."""
    ...
    def _collect_video_clips(self, script: dict, project_dir: Path) -> list[dict[str, Any]]:
        """Extract the list of completed video clips from a script."""
        ...
        for item in items:
            ...
            if not abs_path.is_relative_to(project_dir.resolve()):
                logger.warning("Skipped video_clip outside the project directory: %s", video_clip)
                continue
    ...
    def _resolve_canvas_size(self, project: dict, first_video_path: Path | None = None) -> tuple[int, int]:
        """Resolve the canvas size from the project aspect_ratio or detect it from the first video."""
    ...
    # ------------------------------------------------------------------
    # Internal helpers: draft generation
    # ------------------------------------------------------------------
    ...
    def _generate_draft(
        ...
        *,
        ...
        """Generate the draft files inside ``draft_dir`` using pyJianYingDraft."""
        ...
        offset_us = 0
        for clip in clips:
            # Read the actual video duration first.
    ...
    def _replace_paths_in_draft(self, *, json_path: Path, tmp_prefix: str, target_prefix: str) -> None:
        """Safely replace temporary paths inside draft_content.json."""
        real = os.path.realpath(json_path)
        tmp = os.path.realpath(tempfile.gettempdir()) + os.sep
        if not real.startswith(tmp):
            raise ValueError(f"Refusing to write outside the temp directory: {real}")
    ...
    def export_episode_draft(
        ...
        *,
        ...
        """
        Export the Jianying draft ZIP for the requested episode.

        Returns:
            Path to the ZIP file. This is a temporary file and the caller is responsible for cleanup.

        Raises:
            FileNotFoundError: Project or script does not exist.
            ValueError: No exportable video clips were found.
        """
        ...
        if not clips:
            raise ValueError(f"Episode {episode} has no completed video clips yet. Generate the videos first")

        # 3. Resolve the canvas size, falling back to the first video if aspect_ratio is missing.
        width, height = self._resolve_canvas_size(project, clips[0]["abs_path"])

        # 4. Create the temp directory and copy assets into staging.
        ...
        try:
            ...
            # 5. Generate the draft. create_draft will recreate draft_dir.
            ...
            # 6. Move the assets into the draft directory.
            ...
            # 7. Post-process paths: staging path -> user's local target path.
