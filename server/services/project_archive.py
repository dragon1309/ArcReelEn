...
class ProjectArchiveService:
        ...
        *,
        ...
        if conflict_policy not in {"prompt", "rename", "overwrite"}:
            raise ProjectArchiveValidationError(
                ...
                errors=[f"conflict_policy must be one of prompt, rename, or overwrite; received: {conflict_policy}"],
            )
        ...
        *,
        ...
        for entry in excluded_entries:
            diagnostics.add(
                ...
                f"Non-standard top-level directory/file '{entry}' was excluded from the export",
                location=entry,
            )
    ...
    def _repair_project_tree(self, project_dir: Path) -> ArchiveDiagnostics:
        ...
        if project is None:
            diagnostics.add(
                ...
                f"Could not parse {self.project_manager.PROJECT_FILE}: {project_path}",
            ...
            for index, episode_meta in enumerate(episodes):
                ...
                if script_payload is None:
                    diagnostics.add(
                        "blocking",
                        "invalid_script_json",
                        f"Could not parse script file: {script_path_rel}",
                        location=script_location,
                    )
                    continue
        ...
        *,
        ...
        if isinstance(novel, dict) and "source_file" in novel:
            ...
            diagnostics.add(
                ...
                "novel.source_file field is deprecated and was removed",
        ...
        for deprecated_field in ("characters_in_episode", "clues_in_episode"):
            if deprecated_field in script_payload:
                ...
                diagnostics.add(
                    ...
                    f"{deprecated_field} field is deprecated (now computed at read time) and was removed",
            ...
            if isinstance(clues, list):
                ...
                if missing_clues:
                    diagnostics.add(
                        ...
                        (
                            f"{items_key}[{index}]: {clues_field} references clues missing from "
            ...
            if (
                ...
                if self._materialize_current_file(
                    ...
                    diagnostics.add(
                        ...
                        f"{location}: from {resolved_raw} restored current file {canonical_rel}",
                        location=location,
                    )
                    return True
            return False
            ...
            if version_rel:
                if self._materialize_current_file(
                    ...
                    diagnostics.add(
                        ...
                        f"{location}: from {version_rel} restored current file {canonical_rel}",
                        location=location,
                    )
                    return True
    ...
    def _load_json_file(self, path: Path) -> dict[str, Any] | None:
        ...
        try:
            ...
            return None
        logger.warning("Path escapes project scope; read denied: %s", real)
        return None
    ...
    def _write_json_file(self, path: Path, payload: dict[str, Any]) -> None:
        ...
        if real.startswith(tmp):
            ...
            with open(real, "w", encoding="utf-8") as handle:  # noqa: PTH123
                json.dump(payload, handle, ensure_ascii=False, indent=2)
            return
        raise ValueError(f"Path escapes project scope; write denied: {real}")
    ...
    def _scan_archive_members(self, archive: zipfile.ZipFile) -> list[ArchiveMember]:
        ...
        for info in archive.infolist():
            ...
            if stat.S_ISLNK(mode):
                raise ProjectArchiveValidationError(
                    "Import archive validation failed",
                    errors=[f"ZIP contains a symbolic-link entry: {info.filename}"],
                )
    ...
    def _load_member_json(
        ...
        try:
            ...
            raise ProjectArchiveValidationError(
                "Import archive validation failed",
                errors=[f"Could not parse {label}: {'/'.join(member.parts)}"],
            ) from exc
    ...
    def _locate_project_root(
        ...
        if manifest_members:
            ...
            if len(root_candidates) != 1:
                raise ProjectArchiveValidationError(
                    "Import archive validation failed",
                    errors=["ZIP contains multiple arcreel-export.json files, so the project root cannot be determined"],
                )
        ...
        if not root_candidates:
            raise ProjectArchiveValidationError(
                "Import archive validation failed",
                errors=["project.json was not found in the ZIP archive"],
            )
        if len(root_candidates) != 1:
            raise ProjectArchiveValidationError(
                "Import archive validation failed",
                errors=["ZIP contains multiple project.json files, so the project root cannot be determined"],
            )
    ...
    def _extract_archive_root(
        ...
        for member in members:
            ...
            try:
                ...
                raise ProjectArchiveValidationError(
                    "Import archive validation failed",
                    errors=[f"Extracted path escapes the staging directory: {'/'.join(member.parts)}"],
                ) from exc
        ...
        *,
        ...
        if conflict_policy == "prompt":
            if target_dir.exists():
                raise ProjectArchiveValidationError(
                    "Project identifier conflict detected",
                    status_code=409,
                    errors=[f"Project identifier '{preferred_name}' already exists. Choose overwrite or auto-rename for the import."],
