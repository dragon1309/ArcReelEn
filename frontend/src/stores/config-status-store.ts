...
async function getConfigIssues(): Promise<ConfigIssue[]> {
  ...
  if (!settings.anthropic_api_key?.is_set) {
    issues.push({
      key: "anthropic",
      tab: "agent",
      label: "ArcReel Agent API Key (Anthropic) is not configured",
    });
  }
  ...
  if (!hasMediaType("video")) {
    issues.push({
      ...
      tab: "providers",
      label: "No provider is configured for video generation",
    });
  }
  if (!hasMediaType("image")) {
    issues.push({
      ...
      tab: "providers",
      label: "No provider is configured for image generation",
    });
  }
  if (!hasMediaType("text")) {
    issues.push({
      ...
      tab: "providers",
      label: "No provider is configured for text generation",
    });
  }
