...
describe("SystemConfigPage", () => {
  ...
  it("renders all 5 sidebar sections", () => {
    ...
    expect(screen.getByRole("button", { name: /Media Models/ })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Usage Statistics/ })).toBeInTheDocument();
  ...
  it("clicking Media Models makes it the active section", async () => {
    renderPage();
    const mediaButton = screen.getByRole("button", { name: /Media Models/ });
  ...
  it("clicking Usage Statistics makes it the active section", async () => {
    renderPage();
    const usageButton = screen.getByRole("button", { name: /Usage Statistics/ });
  ...
  it("shows config warning banner when there are config issues", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("The following required configuration is still incomplete:")).toBeInTheDocument();
    });
    expect(
  ...
  it("does not show warning banner when config is complete", async () => {
    ...
    expect(screen.queryByText("The following required configuration is still incomplete:")).not.toBeInTheDocument();
  });
