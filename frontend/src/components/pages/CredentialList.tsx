...
const CredentialRow = memo(function CredentialRow({ cred, providerId, isVertex, onChanged }: RowProps) {
  ...
  const handleActivate = useCallback(async () => {
    try {
      ...
      // Ignore transient network errors; the user can retry
    }
    ...
    >
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={cred.is_active ? "Currently active" : `Activate ${cred.name}`}
      ...
      {editing && (
        <div className="mt-2.5 ml-5.5 space-y-2.5 rounded-lg border border-gray-800 bg-gray-950/60 p-3">
          ...
          <div>
            <label htmlFor={`${editPrefix}-apikey`} className="mb-1 block text-xs text-gray-500">API Key (leave blank to keep current value)</label>
            <input
          ...
          {providerId === "gemini-aistudio" && (
            <div>
              ...
              <input
                ...
                placeholder="Use the official endpoint by default..."
                className={inputClsPlaceholder}
              />
            </div>
          )}
...
function AddCredentialForm({ providerId, isVertex, onCreated, onCancel }: AddFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isVertex) {
        ...
        if (!file) {
          setError("Please choose a credential file");
          setSaving(false);
          return;
        }
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-950/60 p-3 space-y-2.5">
      ...
      {isVertex ? (
        <div>
          <label htmlFor="cred-add-file" className="mb-1 block text-xs text-gray-500">Credential file <span className="text-rose-400">*</span></label>
          <button
            id="cred-add-file"
            type="button"
          ...
          >
            ...
            {fileRef.current?.files?.[0]?.name ?? "Choose JSON file..."}
          </button>
...
export function CredentialList({ providerId, onChanged }: Props) {
  ...
  // Keep onChanged in a ref so refresh stays stable and avoids parent-driven loops
  ...
  // After a user action: refresh the list and notify the parent
    ...
    <div>
      ...
      {credentials.length === 0 && !showAdd && (
        <div className="rounded-lg border border-dashed border-gray-700 px-4 py-6 text-center">
          ...
          >
            <Plus className="h-3 w-3" /> Add your first credential
          </button>
        </div>
      )}
