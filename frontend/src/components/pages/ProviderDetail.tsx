    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder={field.is_set ? field.value_masked ?? "••••••••••" : (field.placeholder ?? "Enter secret")}
        ...
        {field.is_set && !(field.key in draft) && (
          <p className="mt-1 text-xs text-gray-600">Already set. Leave blank to keep the current value.</p>
        )}
      </div>
    );
  }
...
export function ProviderDetail({ providerId, onSaved }: Props) {
  ...
  const handleCredentialChanged = useCallback(async () => {
    // Refresh config quietly so the panel does not flash and remount.
