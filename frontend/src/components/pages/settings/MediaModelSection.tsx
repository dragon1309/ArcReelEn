...
export function MediaModelSection() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Media model settings saved", "success");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 p-6">
      {/* Section heading */}
      <div>
        <h3 className="text-lg font-semibold text-gray-100">Media Models</h3>
        <p className="mt-1 text-sm text-gray-500">Set global default generation models. Project-level overrides are still supported.</p>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        ...
        {videoBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            No video provider is available yet. Configure API keys in the Providers section first.
          </div>
        )}
        ...
        <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm text-gray-300">
          ...
          />
          Generate audio
          <span className="text-xs text-gray-500">(supported by select providers only)</span>
        </label>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        ...
        {imageBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            No image provider is available yet. Configure API keys in the Providers section first.
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        ...
        <p className="mb-3 text-xs text-gray-500">Choose text models by task type. Leave blank to auto-select.</p>
        ...
        {textBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            No text provider is available yet. Configure API keys in the Providers section first.
          </div>
        )}
      </div>
