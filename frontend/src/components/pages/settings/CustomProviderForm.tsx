...
export function CustomProviderForm({ existing, onSaved, onCancel }: CustomProviderFormProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (enabledModels.length === 0) {
      showError("Enable at least one model");
      return;
    }
    const emptyId = enabledModels.find((m) => !m.model_id.trim());
    if (emptyId) {
      showError("Every enabled model must have a model_id");
      return;
    }
    setSaving(true);
    try {
      if (isEdit && existing) {
        // Atomically update the provider and its models in one request
  ...
  const urlPreview = (() => {
    ...
    if (apiFormat === "openai") {
      // The OpenAI SDK expects a /v1 suffix; the backend adds it automatically
      const base = trimmed.match(/\/v\d+$/) ? trimmed : `${trimmed}/v1`;
      return `${base}/models`;
    }
    // The Google SDK appends /v1beta automatically; the backend strips user-supplied version paths
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={isEdit ? existing?.api_key_masked ?? "Leave blank to keep the current key" : "Enter API Key"}
          ...
          <div>
            <div className="mb-2 flex items-center gap-3 text-sm text-gray-400">
              ...
              {models.length > 1 && (
                ...
                >
                  {filteredModels.every((m) => m.is_enabled) ? "Clear selection" : "Select all"}
                </button>
              )}
            </div>
            {models.length > 5 && (
        ...
        {models.length === 0 && (
          <div className="rounded-xl border border-dashed border-gray-700 p-4 text-center text-sm text-gray-500">
            Click "Fetch Model List" to auto-discover models, or
            <button
              type="button"
              onClick={addManualModel}
