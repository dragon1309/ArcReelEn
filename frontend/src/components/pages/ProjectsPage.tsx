    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 px-4">
      <div className="w-full max-w-md rounded-2xl border border-amber-400/20 bg-gray-900 p-6 shadow-2xl shadow-black/40">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm leading-6 text-gray-400">
              The import bundle wants to use project ID
              <span className="mx-1 rounded bg-gray-800 px-1.5 py-0.5 font-mono text-gray-200">
                {projectName}
              </span>
              . That ID already exists. You can overwrite the existing project or auto-rename the import and continue.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3">
          ...
          >
            <span>
              ...
              <span className="mt-1 block text-xs text-red-200/80">
                Replace the current project data with the imported bundle.
              </span>
            </span>
          ...
          >
            <span>
              <span className="block font-medium">Auto-Rename Import</span>
              <span className="mt-1 block text-xs text-indigo-200/80">
                Keep the existing project and generate a new internal ID for the imported copy.
              </span>
            </span>
            {importing && <Loader2 className="h-4 w-4 animate-spin" />}
          </button>
...
function ImportDiagnosticsDialogWrapper({
  ...
  return (
    <ArchiveDiagnosticsDialog
      ...
      description="Import preflight has completed. Issues are grouped by severity below. Blocking issues must be resolved before the import can continue."
      sections={[
    ...
    async (
      ...
      try {
        ...
        useAppStore.getState().pushToast(
          autoFixedCount > 0
            ? `Imported "${result.project.title || result.project_name}" with ${autoFixedCount} auto-fix${autoFixedCount === 1 ? "" : "es"}`
            : `Imported "${result.project.title || result.project_name}"`,
          "success"
        );
        if (warningCount > 0) {
      ...
      <header className="border-b border-gray-800 px-6 py-4">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <h1 className="flex items-center gap-2 text-xl font-bold">
            ...
            <span className="ml-1 text-base font-normal text-gray-400">Projects</span>
          </h1>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {projectsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="text-sm mt-1">Create a new project or import a ZIP file to get started</p>
          </div>
        ) : (
