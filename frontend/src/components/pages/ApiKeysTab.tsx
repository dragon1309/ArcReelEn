 ...
 * Shows the list, creation modal, and revoke confirmation modal.
 */
...
function CreateModal({ onClose, onCreated }: CreateModalProps) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setCreating(true);
    try {
      // Send 0 for empty/zero to indicate "never expires".
      ...
      useAppStore.getState().pushToast(`Failed to create API key: ${(err as Error).message}`, "error");
    } finally {
      setCreating(false);
    }
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2.5 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-3">
                ...
                <p className="text-xs leading-5 text-amber-200">
                  Copy and store this API key now. For security reasons, the full key
                  <strong className="font-semibold"> is shown only once</strong> and cannot be viewed again after closing this dialog.
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Example: OpenClaw Integration"
                  autoFocus
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-700 bg-gray-950 px-3 py-2.5 text-sm text-gray-200 placeholder:text-gray-600 focus:border-indigo-500/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40"
                />
                <p className="mt-1 text-xs text-gray-600">Defaults to 30 days. Leave empty for no expiration.</p>
              </div>
...
function DeleteModal({ keyInfo, onClose, onDeleted }: DeleteModalProps) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to revoke API key: ${(err as Error).message}`, "error");
    } finally {
      setDeleting(false);
    }
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs leading-5 text-gray-400">
                ...
                Any service using this key will immediately lose access, and the action cannot be undone.
              </p>
            </div>
          </div>
      ...
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Use Bearer tokens to access the ArcReel API from external services like OpenClaw.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/60 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-14 text-gray-600">
            ...
            <p className="text-xs">Click “New Key” to create your first one</p>
          </div>
        ) : (
