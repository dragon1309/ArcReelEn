...
export function VersionTimeMachine({
  ...
  async function handleRestore(version: number) {
    ...
    try {
      ...
      useAppStore
        .getState()
        .pushToast(`Failed to switch versions: ${(err as Error).message}`, "error");
    } finally {
      setRestoringVersion(null);
    }
  }
          ...
          >
            {loading ? (
              ...
              <div className="space-y-1">
                <p className="text-[11px] font-medium text-gray-300">No saved versions yet</p>
                <p className="text-[11px] leading-5 text-gray-500">
                  Generated or restored versions will appear here.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {!selectedInfo && (
                  <p className="text-[10px] leading-4 text-gray-400">
                    Click a version number to preview it. Any non-current version can be restored.
                  </p>
                )}
                  ...
                  <div className="rounded-xl border border-gray-700 bg-gray-950/80 p-2.5">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      ...
                      {selectedInfo.is_current ? (
                        ...
                        >
                          {restoringVersion === selectedInfo.version ? "Switching..." : "Switch to this version"}
                        </button>
                      )}
                    </div>
                    ...
                    <p className="line-clamp-4 text-[11px] leading-5 text-gray-400">
                      {selectedInfo.prompt || "No additional notes were recorded for this version."}
                    </p>
