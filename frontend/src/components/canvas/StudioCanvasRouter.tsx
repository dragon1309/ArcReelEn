...
export function StudioCanvasRouter() {
  ...
  // Derive loading state from the task queue (instead of local state)
  ...
  const handleUpdatePrompt = useCallback(async (segmentId: string, field: string, value: unknown, scriptFile?: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to update the prompt: ${(err as Error).message}`, "error");
    }
  ...
  const handleGenerateStoryboard = useCallback(async (segmentId: string, scriptFile?: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to generate the storyboard: ${(err as Error).message}`, "error");
    }
  ...
  const handleGenerateVideo = useCallback(async (segmentId: string, scriptFile?: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to generate the video: ${(err as Error).message}`, "error");
    }
  }, [currentProjectName, currentScripts]);
  ...
  const handleSaveCharacter = useCallback(async (
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to update the character: ${(err as Error).message}`, "error");
    }
  ...
  const handleUpdateClue = useCallback(async (name: string, updates: Partial<Clue>) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Failed to update the clue: ${(err as Error).message}`, "error");
    }
  }, [currentProjectName, refreshProject]);
